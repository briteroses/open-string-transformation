...
from models.black_box_model import GPTFamily, HaizerMistral
...
def smorgasbord_default_hparams():
    return {
        "attack_budget": [20,],
        "ou_ordering": ["obfuscate_before_universal", "universal_before_obfuscate"],
    }
@dataclass
class ObfuscationsPlusUniversalExperiment(BaseExperiment):
    target_model: LanguageModel

    hyperparameter_grid: dict = field(default_factory=smorgasbord_default_hparams)

    all_parameters = ["target_model", "attack_budget", "ou_ordering"]

    name = "adaptive_attack"
    description = "Perform the string composition adaptive attack combined with universal jailbreak injections."
    ...
    def run(self, hyperparam_setting, config):
        ...
        ou_ordering = hyperparam_setting["ou_ordering"]
        ...
        for i in tqdm(range(len(safety_dataset)), desc="bad prompt"):
            ...
            bad_prompt_as_input += "." if bad_prompt_as_input[-1] != "." else ""
            ...
            for trial in tqdm(range(single_attack_progress, attack_budget), desc="attack trial", leave=False):
                ...
                the_bad_prompt = bad_prompt_as_input
                if ou_ordering == "universal_before_obfuscate":

                    for augment in augment_list:
                        the_bad_prompt = augment(the_bad_prompt)

                    if adaptive_sampling_params.composition_target == "query":
                        for transform in transform_list:
                            the_bad_prompt = transform(the_bad_prompt)
                    elif adaptive_sampling_params.composition_target == "response":
                        the_bad_prompt = other_transform(the_bad_prompt)
                    
                elif ou_ordering == "obfuscate_before_universal":
...
class BaselineUniversalExperiment(BaseExperiment):
    ...
    hyperparameter_grid: dict = field(default_factory=baseline_empty_hparams)
